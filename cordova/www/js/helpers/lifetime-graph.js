...
function lifeTimeResults(roundResults) {
  ...
  var yAxis = d3.axisRight(yScale).ticks(1).tickFormat(function (d) {
    return d + "%";
  });
  ...
  var zoom = d3.zoom().scaleExtent([1, 1]).translateExtent([[0, -10], [width + 80, height]]).on("zoom", zoomed);

  function zoomed() {
    chart.attr("transform", "translate(" + d3.event.transform.x + ", 0)");
  }

  // Append SVG to div

  var svg = d3.select("#lifetime-results-page__lifetime-graph").append("svg").attr("width", "100%").attr("height", height + margin.top + margin.bottom).call(zoom).append("g").attr("transform", "translate(" + margin.left + ", " + margin.top + ")");
  ...
  svg.append("g").attr("transform", "translate(-60, 0)").attr("class", "lifetime-graph__axis").call(yAxis);
